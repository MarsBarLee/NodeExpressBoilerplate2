...
class Users extends Component {
  ...
  render() {
    if (this.state.users.length) {
          ...
          <div className="current-users">
            ...
            <div>
              {this.state.users.map(el => {
                return <div key={el.userid}>{el.username}</div>;
              })}
            </div>
          </div>
      ...
      return (
        <div>
          503 Service Unavailable. If you see this message, that our servers are
          temporarily unavailable. Our apologies, please refresh or come back
          another time.
        </div>
      );
    }
